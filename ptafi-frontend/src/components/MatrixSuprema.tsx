...
export const MatrixSuprema = ({ analysis }: MatrixSupremaProps) => {
    ...
    const handleDownloadWord = () => {
        if (!analysis.docx_base64) {
            alert("El informe Word no está listo o hubo un error generándolo.");
            return;
        }

        const linkSource = `data:application/vnd.openxmlformats-officedocument.wordprocessingml.document;base64,${analysis.docx_base64}`;
        const downloadLink = document.createElement("a");
        const fileName = `Informe_Auditoria_${analysis.institution_info.name.replace(/\s+/g, '_')}.docx`;
                    ...
                    <aside className="space-y-8">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            ...
                            <button
                                ...
                                className="w-full relative group"
                            >
                                <div className="absolute -inset-0.5 bg-gradient-to-r from-emerald-600 to-teal-600 rounded-2xl blur opacity-30 group-hover:opacity-100 transition duration-500"></div>
                                <div className="relative px-8 py-5 bg-[#020205] border border-white/10 rounded-2xl flex items-center justify-center gap-3 font-bold text-sm tracking-widest uppercase transition-all duration-300 group-hover:bg-black/80">
